...
- [Storage Selection Boundary](#storage-selection-boundary)
...
## Storage Selection Boundary

Use SwiftData for structured, queryable app data when the deployment target and
schema constraints fit. Route adjacent persistence work deliberately:

| Data need | Prefer | Route to |
|---|---|---|
| User preferences, feature flags, small scalar settings | `UserDefaults` / `@AppStorage` | `swiftui-patterns` when SwiftUI state binding matters |
| Passwords, tokens, API keys, signing keys | Keychain | `swift-security` |
| User-visible documents, images, videos, PDFs, exports | FileManager / document picker patterns | `ios-networking` file-storage reference when sandbox directories or protection matter |
| Existing Core Data stack with no SwiftData adoption | Core Data | `core-data` |
| Core Data + SwiftData coexistence or migration | SwiftData migration planning | this skill plus `references/core-data-coexistence.md` |
| Public/shared CloudKit data or advanced CloudKit workflows | CloudKit-specific architecture | `cloudkit` |
| Full SQL control, custom indexes, raw query plans, cross-platform SQLite | SQLite/GRDB-style design | outside this collection unless the app already uses it |

Do not turn every persistence question into SwiftData. First classify the data
by sensitivity, size, ownership, query needs, sync model, and deployment target.
