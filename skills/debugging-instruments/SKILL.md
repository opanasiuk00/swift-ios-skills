...
Start with the symptom, pick the narrowest tool, then verify on a real device
for performance claims.

| Symptom | First tool | Look for |
|---|---|---|
| UI freezes or hangs during an interaction | Time Profiler / Hangs | Main-thread synchronous I/O, decoding, locks, priority inversion |
| Memory grows after repeating a flow | Allocations + Memory Graph | Retained generations, closure cycles, strong delegates, unbounded caches |
| Sudden memory spike | Allocations / VM Tracker | Image decode, bulk fetch/import, large temporary buffers |
| Slow SwiftUI scrolling or redraws | SwiftUI instrument + Time Profiler | Excessive body evaluations, unstable identity, expensive computed work |
| Dropped frames or animation hitches | Animation Hitches / Core Animation | Layout thrash, blending, offscreen rendering, commit/render work |
| Battery drain or thermal pressure | Power Profiler / Energy diagnostics | CPU wakeups, polling, location/network frequency, background work |
| Slow launch | App Launch + Time Profiler | Pre-main dylib/static init cost, post-main synchronous setup |
| Excessive disk or file latency | File Activity | Repeated small writes, main-thread file I/O, cache churn |
| Build failures or slow builds | Build log / Xcode build timeline | First compiler error, dependency resolution, type-checking hotspots |
